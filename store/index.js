  ...
  return new Vuex.Store({
    ...
    actions: {
      ...
      async getUserRessources({ commit, getters }) {
        ...
        const fcTemplate = userTemplates.find(
          (template) => template.template_id === '260676'
        )
        let nbFarmerCoins = 0
        const barleySeedTemplate = userTemplates.find(
          (template) => template.template_id === '298595'
        )
        let nbBarleySeeds = 0
        const barleyTemplate = userTemplates.find(
          (template) => template.template_id === '318606'
        )
        let nbBarleys = 0
        const cornSeedTemplate = userTemplates.find(
          (template) => template.template_id === '298596'
        )
        let nbCornSeeds = 0
        const cornTemplate = userTemplates.find(
          (template) => template.template_id === '318607'
        )
        let nbCorns = 0
        const chickenEggsTemplate = userTemplates.find(
          (template) => template.template_id === '298612'
        )
        let nbChickenEggs = 0
        const milkTemplate = userTemplates.find(
          (template) => template.template_id === '298593'
        )
        let nbMilks = 0
      ...
      async getUserTools({ commit, getters }) {
        const toolsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        const toolConfsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        tools.forEach((tool) => {
          const toolConf = toolConfs.find(
            (toolConf) => toolConf.template_id === tool.template_id
          )
          tool.name = toolConf.template_name
          tool.img_url = 'https://ipfs.atomichub.io/ipfs/' + toolConf.img
          tool.isTool = true
          tool.charged_time = toolConf.charged_time
          let savedClaims = 0
          if (tool.type === 'Wood') savedClaims = getters.woodSavedClaims
          if (tool.type === 'Food') savedClaims = getters.foodSavedClaims
          if (tool.type === 'Gold') savedClaims = getters.goldSavedClaims
          tool.stored_claims = (savedClaims + 1) - Math.ceil((tool.next_availability + savedClaims * tool.charged_time - Math.floor(Date.now() / 1000)) / tool.charged_time)
        })
      ...
      async getUserMbs({ commit, getters }) {
        const mbsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        const mbsConfsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        mbs.forEach((mb) => {
          const mbsConf = mbsConfs.find(
            (mbConf) => mbConf.template_id === mb.template_id
          )
          mb.name = `${mbsConf.name} (${mbsConf.type})`
          mb.img_url = 'https://ipfs.atomichub.io/ipfs/' + mbsConf.img
          mb.saved_claims = mbsConf.saved_claims
        })

        commit('setUserMbs', mbs)
      },
      async getUserCrops({ commit, getters }) {
        const cropsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        const cropConfsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        crops.forEach((crop) => {
          const cropConf = cropConfs.find(
            (cropConf) => cropConf.template_id === crop.template_id
          )
      ...
      async getUserAnimals({ commit, getters }) {
        const animalsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        const animalConfsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        const fwTemplates = (
          await this.$axios.$get(
            'https://wax.api.atomicassets.io/atomicassets/v1/templates?collection_name=farmersworld&page=1&limit=500&order=desc&sort=created'
          )
        ).data

        animals.forEach((animal) => {
          const animalConf = animalConfs.find(
            (animalConf) => animalConf.template_id === animal.template_id
          )
          ...
          animal.consumed_card_name =
            animalConf.consumed_card === 0
              ? ''
              : fwTemplates.find(
                  (tpl) => tpl.template_id === animal.consumed_card.toString()
                ).name
          animal.consumed_quantity = animalConf.consumed_quantity
          animal.daily_claim_limit = animalConf.daily_claim_limit
          animal.reward_card_name =
            animalConf.reward_card === 0
              ? ''
              : fwTemplates.find(
                  (tpl) => tpl.template_id === animalConf.reward_card.toString()
                ).name

          if (animal.day_claims_at) {
            if (
              animal.day_claims_at.length === animal.daily_claim_limit &&
              animal.next_availability <
                Math.min(...animal.day_claims_at) + 3600 * 24
            ) {
              animal.next_availability =
                Math.min(...animal.day_claims_at) + 3600 * 24
            }
      ...
      async getUserBuildings({ commit, getters }) {
        const buildingsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        const buildingConfsData = await this.$axios.$post(
          'https://chain.wax.io/v1/chain/get_table_rows',
          {
            json: true,
            code: 'farmersworld',
        ...
        buildings.forEach((building) => {
          const buildingConf = buildingConfs.find(
            (buildingConf) => buildingConf.template_id === building.template_id
          )
          building.name = buildingConf.name
          building.img_url =
            'https://ipfs.atomichub.io/ipfs/' + buildingConf.img
    ...
    getters: {
      ...
      woodSavedClaims(state) {
        return state.userMbs
          .filter((mbs) => mbs.type === 'Wood')
          .reduce((acc, curr) => acc + curr.saved_claims, 0)
      },
      foodSavedClaims(state) {
        return state.userMbs
          .filter((mbs) => mbs.type === 'Food')
          .reduce((acc, curr) => acc + curr.saved_claims, 0)
      },
      goldSavedClaims(state) {
        return state.userMbs
          .filter((mbs) => mbs.type === 'Gold')
          .reduce((acc, curr) => acc + curr.saved_claims, 0)
      },
      userWood(state) {
        if (state.userRessources) {
          const woodRss = state.userRessources.balances.find((rss) =>
            rss.includes('WOOD')
          )
          if (!woodRss) {
            return 0
      ...
      userFood(state) {
        if (state.userRessources) {
          const foodRss = state.userRessources.balances.find((rss) =>
            rss.includes('FOOD')
          )
          if (!foodRss) {
            return 0
      ...
      userGold(state) {
        if (state.userRessources) {
          const goldRss = state.userRessources.balances.find((rss) =>
            rss.includes('GOLD')
          )
          if (!goldRss) {
            return 0
      ...
      userFc(state) {
        if (state.userRessources) {
          const fcRss = state.userRessources.balances.find((rss) =>
            rss.includes('FC')
          )
          if (!fcRss) {
            return 0
      ...
      userBarleys(state) {
        if (state.userRessources) {
          const barleyRss = state.userRessources.balances.find((rss) =>
            rss.includes('BARLEYS')
          )
          if (!barleyRss) {
            return 0
          }
      ...
      userBarleySeeds(state) {
        if (state.userRessources) {
          const barleySeedRss = state.userRessources.balances.find((rss) =>
            rss.includes('BARLEY SEEDS')
          )
          if (!barleySeedRss) {
            return 0
          }
      ...
      userCorns(state) {
        if (state.userRessources) {
          const cornRss = state.userRessources.balances.find((rss) =>
            rss.includes('CORNS')
          )
          if (!cornRss) {
            return 0
      ...
      userCornSeeds(state) {
        if (state.userRessources) {
          const cornSeedRss = state.userRessources.balances.find((rss) =>
            rss.includes('CORN SEEDS')
          )
          if (!cornSeedRss) {
            return 0
          }
      ...
      userChickenEggs(state) {
        if (state.userRessources) {
          const chickenEggRss = state.userRessources.balances.find((rss) =>
            rss.includes('CHICKEN EGGS')
          )
          if (!chickenEggRss) {
            return 0
          }
      ...
      userMilks(state) {
        if (state.userRessources) {
          const MilkRss = state.userRessources.balances.find((rss) =>
            rss.includes('MILKS')
          )
          if (!MilkRss) {
            return 0
      ...
      userFarmPlots(state) {
        if (state.userBuildings) {
          const farmPlots = state.userBuildings.filter(
            (bld) => bld.template_id === 298592
          )
          if (!farmPlots) {
            return []
          }
